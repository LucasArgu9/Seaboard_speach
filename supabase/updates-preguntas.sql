...
from (values
  ...
  ('q36', '¿Qué carrera se puede vincular con el Área Comercial de Seaboard (exportaciones de azúcar, alcohol y bioetanol)?', 'media', 'Cuadro Carreras y Áreas de Seaboard: Área Comercial → exportaciones de azúcar, alcohol y bioetanol.', 'q36a'),
...
from (values
  ...
  ('q02d', 'Papel y cartón', 0),
  ('q02a', 'Azúcar, bioetanol y energía', 1),
  ('q02b', 'Solamente azúcar', 2),
  ('q02c', 'Vino y aceite de oliva', 3),
  ('q03b', 'Rosario de la Frontera', 0),
  ('q03c', 'Cafayate', 1),
  ('q03a', 'Orán', 2),
  ('q03d', 'General Güemes', 3),
  ('q04b', 'Jujuy', 0),
  ('q04c', 'Tucumán', 1),
  ('q04d', 'Córdoba', 2),
  ('q04a', 'Salta', 3),
  ('q05a', 'Más de 2000 personas', 0),
  ('q05b', 'Alrededor de 300 personas', 1),
  ('q05c', 'Menos de 100 personas', 2),
  ('q05d', 'Más de 50000 personas', 3),
  ('q06b', 'Solo directo', 0),
  ('q06a', 'Directo e indirecto', 1),
  ('q06c', 'Solo indirecto', 2),
  ('q06d', 'Solo temporal por un mes', 3),
  ('q07c', 'Moda, música y deporte', 0),
  ('q07d', 'Importación, reventa y logística', 1),
  ('q07a', 'Tradición, tecnología y sostenibilidad', 2),
  ('q07b', 'Turismo, minería y pesca', 3),
  ('q08b', 'La soja', 0),
  ('q08c', 'El maíz', 1),
  ('q08d', 'La uva', 2),
  ('q08a', 'La caña de azúcar', 3),
  ('q09a', 'Con el ambiente y con la comunidad', 0),
  ('q09d', 'Con otros países únicamente', 1),
  ('q09b', 'Solo con los accionistas', 2),
  ('q09c', 'Con nada en particular', 3),
  ('q10b', 'En videojuegos', 0),
  ('q10a', 'En bioetanol y energías renovables', 1),
  ('q10c', 'En telefonía celular', 2),
  ('q10d', 'En autos eléctricos', 3),
  ('q11b', 'Una vez cada diez años', 0),
  ('q11c', 'Nunca', 1),
  ('q11a', 'De forma permanente', 2),
  ('q11d', 'Solo a los gerentes', 3),
  ('q12c', 'Ninguna', 0),
  ('q12d', 'Una hora por persona', 1),
  ('q12b', 'Cinco horas', 2),
  ...
  ('q14d', 'Pasa obligatoriamente a otra empresa', 0),
  ('q14a', 'Puede proyectar su carrera dentro de la empresa', 1),
  ('q14b', 'Debe irse apenas termina', 2),
  ('q14c', 'No vuelve a tener contacto con Seaboard', 3),
  ('q15b', 'Sí, mínimo 10 años', 0),
  ('q15c', 'Sí, siempre es obligatoria', 1),
  ('q15a', 'No: también apuestan al talento joven', 2),
  ('q15d', 'Sí, y debe ser en el exterior', 3),
  ('q16b', 'Trabajar sin aprender nada nuevo', 0),
  ('q16c', 'Solo observar sin participar', 1),
  ('q16d', 'Reemplazar por completo a la universidad', 2),
  ...
  ('q18d', 'Nada más que el sueldo', 0),
  ('q18a', 'Capacitaciones permanentes y desarrollo real', 1),
  ('q18b', 'Un auto 0 km a cada ingresante', 2),
  ('q18c', 'Vacaciones de seis meses', 3),
  ('q19d', 'Solo en otros continentes', 0),
  ('q19b', 'Solamente en la bolsa de valores', 1),
  ('q19a', 'La comunidad y el medio ambiente', 2),
  ('q19c', 'En nada concreto', 3),
  ('q20c', 'Igual que en una megaciudad', 0),
  ('q20d', 'Sin ningún servicio', 1),
  ('q20b', 'Muy estresante', 2),
  ('q20a', 'Tranquila', 3),
  ('q21a', 'Colonial', 0),
  ('q21b', 'Futurista', 1),
  ('q21c', 'Medieval', 2),
  ('q21d', 'Industrial moderno', 3),
  ('q22b', 'Únicamente ajedrez', 0),
  ('q22a', 'Gym, tenis, vóley, básquet, natación y golf, entre otras', 1),
  ('q22c', 'Ninguna actividad deportiva', 2),
  ('q22d', 'Solo esquí', 3),
  ('q23b', 'Es el más pequeño del país', 0),
  ('q23c', 'Está siempre cerrado', 1),
  ('q23a', 'Es uno de los mejores de la región', 2),
  ('q23d', 'No tiene césped', 3),
  ('q24b', 'Solo una oficina administrativa', 0),
  ('q24c', 'Un aeropuerto internacional', 1),
  ('q24d', 'Nada, hay que salir del complejo para todo', 2),
  ('q24a', 'Súper, servicio médico, cajeros automáticos y restorán', 3),
  ('q25a', 'Sí: hay pasantes que hoy ocupan cargos importantes', 0),
  ('q25c', 'Solo si te vas y volvés', 1),
  ('q25d', 'Solo con contactos externos', 2),
  ('q25b', 'No, nadie asciende nunca', 3),
  ('q26b', 'A traer siempre gente de afuera', 0),
  ('q26a', 'Al desarrollo interno', 1),
  ('q26c', 'A dejar los cargos vacantes', 2),
  ...
  ('q28b', 'info@salta.gob.ar', 0),
  ('q28c', 'cv@universidad.edu', 1),
  ...
  ('q30c', 'Cosecha', 3),
  ('q31c', 'Psicología', 0),
  ('q31d', 'Lic. en Comercio Internacional', 1),
  ('q31a', 'Ingeniería Industrial', 2),
  ('q31b', 'Abogacía', 3),
  ('q32d', 'Proyectos Hídricos', 0),
  ('q32b', 'Cultivo y Cosecha de caña', 1),
  ('q32c', 'Relaciones Laborales', 2),
  ...
  ('q34d', 'Service Desk y Redes', 0),
  ('q34a', 'Contraloría, Finanzas, Administración y Recursos Humanos', 1),
  ('q34b', 'Calderas y Cogeneración', 2),
  ('q34c', 'Herbicidas y Cosecha', 3),
  ...
  ('q36b', 'Ingeniería Química', 0),
  ('q36d', 'Ingeniería Civil', 1),
  ('q36c', 'Licenciatura en Enfermería', 2),
  ('q36a', 'Licenciatura en Administración', 3),
  ('q37a', 'Ingeniería Civil', 0),
  ('q37b', 'Psicología', 1),
  ('q37c', 'Abogacía', 2),
  ('q37d', 'Lic. en Recursos Humanos', 3),
  ...
  ('q39b', 'Arquitectura', 0),
  ('q39c', 'Contador Público', 1),
  ('q39a', 'Lic. en Gestión Eficiente de la Energía', 2),
  ('q39d', 'Abogacía', 3),
  ('q40b', 'Tener un título de posgrado', 0),
  ('q40c', 'Vivir en el exterior', 1),
  ('q40d', 'Hablar tres idiomas', 2),
